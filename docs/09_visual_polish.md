...
`.import` 元資料。因此這支視覺分支相對 `master` 是**零刪除**（54 個新增／61 個修改／
0 個刪除）——連上一代角色名單殘留在 `client/assets/agents/` 的 58 個素材也原封不動保留，
要回收請自行 `--clean`。同理，粒子預設**不全部預烘福模擬影格**：只有 `vfx_manager.gd`
